  ...
  (cond ((eq? parent #f) (void))
        ...
        ((node-marked parent)
         (let ((grandparent (node-parent parent)))
          ...
          (set-node-parent! parent #f)
          (add-node-to-dll! h parent)
          (check-parents! h grandparent parent)))))
...
(define (replace-in-dll! node1 node2)
  ...
  (cond ((and (eq? node2 (node-left node2)) (eq? node2 (node-left node2))) ;if node2 is the only node in dll, do nothing
         (void))
        (else (let ((rnode (node-right node2))
